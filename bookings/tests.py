...
class BookingServiceTest(TestCase):
    ...
    def test_validate_valid_booking(self):
        validate_booking_slots(self.court, self.tomorrow, time(10, 0), time(12, 0))
    ...
    def test_validate_overlapping_booking(self):
        ...
        with self.assertRaises(ValidationError):
            validate_booking_slots(self.court, self.tomorrow, time(11, 0), time(13, 0))
...
class BookingQuerySetTest(TestCase):
    def setUp(self):
        ...
        self.court = Court.objects.create(name='Cancha 1', price_per_hour=50.00)
        today = timezone.now().date()
        tomorrow = today + timedelta(days=1)

        Booking.objects.create(
            user=self.user, court=self.court, date=tomorrow,
            start_time=time(10, 0), end_time=time(12, 0),
            total_price=100.00, status='PENDING'
        )
        Booking.objects.create(
            user=self.user, court=self.court, date=today - timedelta(days=1),
            start_time=time(10, 0), end_time=time(12, 0),
